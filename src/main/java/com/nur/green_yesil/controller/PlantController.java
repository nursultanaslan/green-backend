...
import com.nur.green_yesil.repository.PlantRepository;
...
public class PlantController {
    ...
    public ResponseEntity<?> commonName(@PathVariable("commonName") String commonName){
        try {
            return ResponseEntity.ok(plantRepository.findByCommonNameIgnoreCaseContaining(commonName));
        }catch (Exception e){
            return ResponseEntity.badRequest().body("yaygın isme göre bitki getirilemedi. HATA:" + e.getMessage());
        }
    }
    ...
    public ResponseEntity<?> getPlant(){
        try {
            return ResponseEntity.ok(plantRepository.findAllByOrderByHeight());
        }catch (Exception e){
            return ResponseEntity.badRequest().body("uzunluga göre bitki getirilemedi. Hata:" +e.getMessage());
        }
    }
